...
export const RectangularTruss = ({ width, height, position }: RectangularTrussProps) => {
  ...
  const trussElements = useMemo(() => {
    ...
    // Vigas superiores e inferiores
    elements.push(
      // Viga superior frontal
      { pos: [0, trussHeight / 2, -width / 2], args: [barThickness, barThickness, width] as [number, number, number], rotation: [0, 0, 0] as [number, number, number] },
      // Viga superior traseira
      { pos: [0, trussHeight / 2, width / 2], args: [barThickness, barThickness, width] as [number, number, number], rotation: [0, 0, 0] as [number, number, number] },
      // Viga inferior frontal
      { pos: [0, -trussHeight / 2, -width / 2], args: [barThickness, barThickness, width] as [number, number, number], rotation: [0, 0, 0] as [number, number, number] },
      // Viga inferior traseira
      { pos: [0, -trussHeight / 2, width / 2], args: [barThickness, barThickness, width] as [number, number, number], rotation: [0, 0, 0] as [number, number, number] },
    );

    // Montantes verticais e barras laterais
    ...
    // Barras laterais longitudinais
    elements.push(
      { pos: [0, trussHeight / 2, 0], args: [barThickness, barThickness, width] as [number, number, number], rotation: [0, 0, Math.PI / 2] as [number, number, number] },
      { pos: [0, -trussHeight / 2, 0], args: [barThickness, barThickness, width] as [number, number, number], rotation: [0, 0, Math.PI / 2] as [number, number, number] },
    );

    // Diagonais para reforço estrutural
    for (let i = 0; i < numVerticals - 1; i++) {
      const z1 = -width / 2 + i * spacing;
      const z2 = -width / 2 + (i + 1) * spacing;
      const zMid = (z1 + z2) / 2;
      const diagonalLength = Math.sqrt(Math.pow(spacing, 2) + Math.pow(trussHeight, 2));
      const angle = Math.atan2(trussHeight, spacing);
